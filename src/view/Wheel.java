...
public class Wheel extends Circle {
    ...
    public Wheel(PisteView pisteView, double x, double y, double radius) {
        super(pisteView, x, y, radius);
    ...
    private void updateConstraints() {
        ...
        for (Capsule capsule : pisteView.getCapsules()) {
            if (capsule.collides(this, vec2Tmp2)) {
                ...
                //velocity.set(getPosition());
                //velocity.sub(previousPosition);
                velocity.sub(PisteView.GRAVITY);
                ...
                if (accelerating) {
                    if (Math.abs(v - desiredSpeed) > 1) {
                        skidding = true;
                        pisteView.spawnPuff((int) getPosition().getX(), (int) (getPosition().getY() + getRadius()), velocity);
                    }
                    v = desiredSpeed;
                }
                else {
